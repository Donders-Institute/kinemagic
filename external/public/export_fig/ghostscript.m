...
% Hacked by Lennart Verhagen (DCCN), lines 50, 77, 130
...
if ispc
    ...
    %% HACK BY DCCN
    %bin = 'gs';
    bin = 'not the right version of gs on the path';
    %% END HACK
end
...
if ispc
    ...
    %path = '/opt/ghostscript/bin/gs';
    path = '/usr/bin/ghostscript';
    %% END HACK
end
...
%% HACK BY DCCN
if ispc
    current_gs_path_str = '/usr/bin/ghostscript';
else
    current_gs_path_str = 'not the right version of gs on the path';
end
%% END
return